  ...
  $.fn.inputHistory = function(opts) 
  {
    ...
    return this.each(function() 
    {
      ...
      var self =
      {
        initialize: function() 
        {
          $container.delegate('#' + $container.attr('id') + ' > div:not(.teach)', 'click', self.  clicked); 
        },
        add: function(opts)
        {
          var $item = $('<div>').addClass(opts.type);
          if (opts.text) { $item.append($('<span>').text(opts.text)); }
          if (opts.html) { $item.append($('<span>').html(opts.html)); }
          if (opts.time)
          {
        ...
        selectPrevious: function()
        {
          ...
          if ($item.is('.teach')) { self.selectPrevious(); return; }
          $item.click();
        ...
        selectNext: function()
        {
          ...
          --index;
          var $item = $container.children(':eq(' + i + ')');
          if ($item.is('.teach')) { self.selectNext(); return; }
          $item.click();
        }
      };
